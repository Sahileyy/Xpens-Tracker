  ...
  return (
    <div className="w-64 bg-white h-screen border-r fixed z-10">
      ...
      <div className="p-4">
        ...
        <ul className="space-y-1 text-gray-950">
          {plans.map((plan: any) => (
            <li key={plan._id} className="relative group flex items-center justify-between">
              
              {/* Plan link */}
              <Link
                href={`/expense/plan/${plan._id}`}
                className="block px-3 py-2 hover:bg-gray-100 rounded flex-1 font-sans font-bold text-2xl"
              >
                {plan.month}
              </Link>
              {/* DELETE BUTTON (added by you) */}
              <DeleteButton id={plan._id.toString()} />
